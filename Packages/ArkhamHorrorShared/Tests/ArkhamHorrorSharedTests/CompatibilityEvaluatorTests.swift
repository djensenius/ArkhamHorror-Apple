...
struct CompatibilityEvaluatorTests {
    /// Evaluator using the compiled-in pin (supportedSchemaRevision 0.1.28, min server 0.1.28,
    /// path /api/v1)
    ...
    private func compatibleServer(
        schemaRevision: ContractRevision = .literal(major: 0, minor: 1, patch: 28),
    ...
    @Test("Server minimum 0.1.0 accepts a client that supports 0.1.28")
    func serverMinimumAcceptsClient() {
        // pin.supportedSchemaRevision 0.1.28 >= server floor 0.1.0: compatible
        let caps = compatibleServer(
    ...
    @Test("Server schema 0.1.29 is accepted when its client floor remains <=0.1.28")
    func higherServerSchemaAccepted() {
        // server 0.1.29 > client minimum 0.1.28: passes serverTooOld check
        // server floor 0.1.28 <= client supports 0.1.28: passes clientTooOld check
        let caps = compatibleServer(
            schemaRevision: .literal(major: 0, minor: 1, patch: 29),
            nativeClientMinimumRevision: .literal(major: 0, minor: 1, patch: 28)
        )
        let outcome = evaluator.evaluate(caps)
        if case .compatible = outcome {} else {
            Issue.record("Expected .compatible for server 0.1.29 with floor 0.1.28, got \(outcome)")
        }
    }
    ...
    @Test("Server minimum 0.1.29 rejects a client that only supports 0.1.28")
    func serverMinimumRejectsClient() {
        let serverMinimum = ContractRevision.literal(major: 0, minor: 1, patch: 29)
        let caps = compatibleServer(nativeClientMinimumRevision: serverMinimum)
        let outcome = evaluator.evaluate(caps)
        #expect(
            outcome == .incompatible(reason: .clientTooOld(
                clientSupports: .literal(major: 0, minor: 1, patch: 28),
    ...
    func serverTooOld() {
        ...
        #expect(
            outcome == .incompatible(reason: .serverTooOld(
                ...
                clientRequires: .literal(major: 0, minor: 1, patch: 28)
            ))
        )
    }

    @Test("Numeric ordering prevents false serverTooOld: 0.1.9 vs 0.1.28")
    ...
    func clientTooOldTakesPrecedence() {
        // client supports 0.1.28 < server floor 0.2.0 -> clientTooOld
        // server schema 0.1.0 < client minimum 0.1.28 -> serverTooOld (not reached)
        let caps = compatibleServer(
