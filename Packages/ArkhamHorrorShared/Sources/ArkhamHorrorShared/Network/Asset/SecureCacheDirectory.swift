...
final class SecureCacheDirectory: @unchecked Sendable {
    ...
    /// The exact on-disk identity/attributes ``attributes(name:)`` reports
    /// for one entry. A named type rather than a bare tuple purely so this
    /// package's own tuple-size lint stays meaningful elsewhere -- every
    /// field here is read by name at every call site already, never
    ...
    func attributes(name: String) throws -> FileAttributes? {
