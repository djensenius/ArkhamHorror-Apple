...
extension AssetDiskCache {
    ...
    /// instance pattern, and is always called immediately alongside it,
    /// as the very first step of every locked entry point in this actor
    /// (``set(_:payload:metadata:token:)``, ``touch(_:metadata:token:)``,
    /// ``remove(_:token:)``, ``removeAll()``, ``beginIssuance(for:)``),
    /// before any of them ever reads or writes the durable clear-epoch
    /// counter.
    ///
